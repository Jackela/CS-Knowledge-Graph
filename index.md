# 计算机科学知识图谱索引 (Computer Science Knowledge Graph Index)

本文档提供整个知识图谱的导航和结构概览。

## 📚 知识领域

### 1. 数据结构 (Data Structures)

线性数据结构：
- [数组](../computer-science/data-structures/array.md)
- [链表](../computer-science/data-structures/linked-list.md)
- [栈](../computer-science/data-structures/stack.md)
- [队列](../computer-science/data-structures/queue.md)
- [哈希表](../computer-science/data-structures/hash-table.md)

树形数据结构：
- [树](../computer-science/data-structures/tree.md)
- [二叉树](../computer-science/data-structures/binary-tree.md)
- [二叉搜索树](../computer-science/data-structures/bst.md)
- [AVL树](../computer-science/data-structures/avl-tree.md)
- [红黑树](../computer-science/data-structures/red-black-tree.md)
- [B树](../computer-science/data-structures/b-tree.md)
- [堆](../computer-science/data-structures/heap.md)
- [Trie](../computer-science/data-structures/trie.md)

图数据结构：
- [图](../computer-science/data-structures/graph.md)
- [并查集](../computer-science/data-structures/disjoint-set.md)

### 2. 算法 (Algorithms)

基础算法：
- [排序](../computer-science/algorithms/sorting.md)
- [二分查找](../computer-science/algorithms/binary-search.md)
- [递归](../computer-science/algorithms/recursion.md)
- [分治](../computer-science/algorithms/divide-conquer.md)

高级算法：
- [动态规划](../computer-science/algorithms/dynamic-programming.md)
- [贪心算法](../computer-science/algorithms/greedy.md)
- [图遍历](../computer-science/algorithms/graph-traversal.md)
- [最短路径](../computer-science/algorithms/shortest-path.md)
- [最小生成树](../computer-science/algorithms/minimum-spanning-tree.md)
- [拓扑排序](../computer-science/algorithms/topological-sort.md)
- [字符串匹配](../computer-science/algorithms/string-matching.md)

### 3. 操作系统 (Operating Systems)

核心概念：
- [操作系统概述](../computer-science/systems/os.md)
- [进程](../computer-science/systems/process.md)
- [线程](../computer-science/systems/thread.md)
- [CPU调度](../computer-science/systems/scheduling.md)
- [进程同步](../computer-science/systems/synchronization.md)
- [死锁](../computer-science/systems/deadlock.md)
- [内存管理](../computer-science/systems/memory-management.md)
- [虚拟内存](../computer-science/systems/virtual-memory.md)
- [文件系统](../computer-science/systems/file-systems.md)

### 4. 计算机网络 (Computer Networks)

网络基础：
- [OSI模型](../computer-science/networks/osi-model.md)
- [物理层](../computer-science/networks/physical-layer.md)
- [数据链路层](../computer-science/networks/data-link-layer.md)
- [网络层](../computer-science/networks/network-layer.md)
- [传输层](../computer-science/networks/transport-layer.md)

核心协议：
- [TCP协议](../computer-science/networks/tcp-protocol.md)
- [HTTP协议](../computer-science/networks/http-protocol.md)
- [HTTPS/TLS](../computer-science/networks/https-tls.md)
- [DNS](../computer-science/networks/dns.md)

### 5. 数据库系统 (Database Systems)

数据库理论：
- [关系模型](../computer-science/databases/relational-model.md)
- [SQL基础](../computer-science/databases/sql-basics.md)
- [SQL高级](../computer-science/databases/sql-advanced.md)
- [规范化](../computer-science/databases/normalization.md)
- [ACID事务](../computer-science/databases/transaction-acid.md)
- [并发控制](../computer-science/databases/concurrency-control.md)
- [索引](../computer-science/databases/indexing.md)

### 6. 分布式系统 (Distributed Systems)

分布式概念：
- [CAP定理](../computer-science/distributed-systems/cap-theorem.md)
- [一致性协议](../computer-science/distributed-systems/consensus.md)
- [分布式事务](../computer-science/distributed-systems/distributed-transactions.md)
- [分片](../computer-science/distributed-systems/sharding.md)
- [负载均衡](../computer-science/distributed-systems/load-balancing.md)

### 7. 软件工程 (Software Engineering)

设计与架构：
- [面向对象设计](../software-engineering/oop-design.md)
- [设计模式](../software-engineering/design-patterns.md)
- [领域驱动设计](../software-engineering/ddd.md)
- [架构模式](../software-engineering/architecture-patterns.md)
- [SOLID原则](../software-engineering/solid-principles.md)

工程实践：
- [单元测试](../software-engineering/testing/unit-testing.md)
- [代码审查](../software-engineering/code-review.md)
- [重构](../software-engineering/refactoring.md)
- [API设计](../software-engineering/api-design.md)

### 8. AI与数据系统 (AI & Data Systems)

机器学习：
- [ML概述](../computer-science/ai-data-systems/ml-overview.md)
- [神经网络](../computer-science/ai-data-systems/neural-networks.md)
- [CNN](../computer-science/ai-data-systems/cnn.md)
- [RNN/LSTM](../computer-science/ai-data-systems/rnn-lstm.md)
- [Transformer](../computer-science/ai-data-systems/transformers.md)

现代AI：
- [LLM](../computer-science/ai-data-systems/llm.md)
- [RAG](../computer-science/ai-data-systems/rag.md)
- [向量数据库](../computer-science/ai-data-systems/vector-db.md)

### 9. 数学基础 (Mathematics)

离散数学：
- [离散数学](../mathematics/discrete-math.md)
- [组合数学](../mathematics/combinatorics.md)
- [图论](../mathematics/graph-theory.md)
- [概率论](../mathematics/probability.md)
- [线性代数](../mathematics/linear-algebra.md)

### 10. 安全 (Security)

安全基础：
- [密码学基础](../security/cryptography-basics.md)
- [认证](../security/authentication.md)
- [授权](../security/authorization.md)
- [常见漏洞](../security/common-vulnerabilities.md)
- [Web安全](../security/web-security.md)

### 11. 产品管理 (Product Management)

- [BRD/PRD/SRS](../product-management/brd-prd-srs.md)
- [MVP](../product-management/mvp.md)
- [敏捷开发](../product-management/agile.md)
- [市场分析](../product-management/market-analysis.md)

### 12. 参考资料 (References)

- [时间复杂度](../references/time-complexity.md)
- [内存寻址](../references/memory-addressing.md)
- [CAP/BASE](../references/cap-base.md)
- [CPU](../references/cpu.md)

## 🎯 学习路线

### 阶段1：工程基础
1. 数据结构：数组 → 链表 → 栈/队列 → 树 → 图
2. 算法：排序 → 二分查找 → 递归 → 动态规划
3. 系统：操作系统基础 → 网络基础

### 阶段2：工程实践
1. 软件工程：设计模式 → 架构 → 测试
2. 数据库：SQL → 索引 → 事务
3. 分布式：CAP → 一致性 → 微服务

### 阶段3：AI系统
1. 机器学习基础
2. 深度学习：CNN → RNN → Transformer
3. 大语言模型与RAG

### 阶段4：进阶主题
1. 编译原理
2. 计算机体系结构
3. 网络安全

## 📊 进度统计

| 领域 | 文档数 | 完成度 |
|------|--------|--------|
| 数据结构 | 15+ | 🟢 核心完成 |
| 算法 | 10+ | 🟢 核心完成 |
| 操作系统 | 8+ | 🟢 核心完成 |
| 计算机网络 | 10+ | 🟢 核心完成 |
| 数据库 | 8+ | 🟢 核心完成 |
| 分布式系统 | 6+ | 🟡 进行中 |
| 软件工程 | 10+ | 🟢 核心完成 |
| AI/数据 | 8+ | 🟡 进行中 |
| 数学基础 | 5+ | 🟡 进行中 |
| 安全 | 5+ | 🟡 进行中 |

## 🔗 交叉引用网络
本知识图谱采用双向链接构建知识网络：

| 连接类型 | 示例链接 |
|---------|---------|
| 数据结构 ↔ 算法 | [数组](../computer-science/data-structures/array.md) ↔ [二分查找](../computer-science/algorithms/binary-search.md) |
| 安全 ↔ 网络 | [HTTPS/TLS](../computer-science/networks/https-tls.md) ↔ [认证](../security/authentication.md) |
| AI ↔ 数学 | [CNN](../computer-science/ai-data-systems/cnn.md) ↔ [线性代数](../mathematics/linear-algebra.md) |
| 系统 ↔ 底层 | [虚拟内存](../computer-science/systems/virtual-memory.md) ↔ [内存寻址](../references/memory-addressing.md) |
| 数据库 ↔ 分布式 | [索引](../computer-science/databases/indexing.md) ↔ [分片](../computer-science/distributed-systems/sharding.md) |

每个概念文件包含「相关概念」章节，按四类组织：
- **数据结构** - 相关的数据组织方式
- **算法** - 相关的计算方法
- **复杂度分析** - 时间和空间复杂度
- **系统实现** - 底层实现与工程实践

本知识图谱采用双向链接：
- 每个概念链接到前置知识
- 每个概念链接到相关扩展
- 面试问题链接到解决方案

## 📝 贡献指南

参见 [CONTRIBUTING.md](../CONTRIBUTING.md) 了解如何贡献内容。

## 📚 推荐资源

- 《算法导论》(CLRS)
- 《操作系统概念》(Silberschatz)
- 《计算机网络：自顶向下方法》(Kurose & Ross)
- 《设计数据密集型应用》(Martin Kleppmann)
- 《深度学习》(Goodfellow)

---

*最后更新：2026年3月*
