...
| 概念 | 关系 | 说明 |
|------|------|------|
| [二叉树](./binary-tree.md) | 特例 | 每个节点最多两个子节点的有序树，是树结构最重要的子类 |
| [图](./graph.md) | 泛化 | 树是连通无环图，是图的特例；去掉无环限制即为一般图 |
| 森林 | 集合 | 多棵互不相交的树的集合，可通过添加虚拟根节点转换为树 |
| 堆 | 应用 | 基于完全二叉树实现的优先队列，满足堆性质 |

### 树的变体与扩展

- **Trie（前缀树）**：用于字符串检索的多叉树
- **线段树**：用于区间查询的二叉树
- **树状数组（BIT）**：利用树结构的数组实现
- **树链剖分**：将树分解为链以支持路径操作
- **重轻分解**：树链剖分的经典算法
...
| 特性 | 树 | 一般图 |
|------|-----|--------|
| 边数 | $n-1$ | 任意（最多 $n(n-1)/2$） |
| 连通性 | 必须连通 | 可有多个连通分量 |
| 环路 | 无环 | 可含环 |
| 路径唯一性 | 任意两点间路径唯一 | 可能存在多条路径 |
| 存储方式 | 父指针、孩子链表等 | 邻接矩阵、邻接表 |
| 遍历复杂度 | $O(n)$ | $O(V+E)$ |
